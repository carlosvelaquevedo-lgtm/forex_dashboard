...
def update_signal_status(signal_id: str, status: str, outcome: str = None, 
...
def get_current_price(symbol: str) -> Optional[float]:
    ...
    # Try OANDA pricing endpoint first
...
def backtest_signal(signal: Dict, future_data: pd.DataFrame) -> Dict:
    ...
    if future_data.empty:
        return {"outcome": "open", "exit_price": None, "pnl_pips": 0}
    ...
    return {"outcome": "open", "exit_price": last_close, "pnl_pips": pnl_pips}
...
def estimate_win_probability(signal: Dict, historical_stats: Dict = None) -> float:
    ...
    - RSI position
    """
    base_prob = 50.0  # Start at 50%
    
    # Adjust for confidence (0-100 maps to ±10%)
...
def format_signals_dataframe(df: pd.DataFrame, columns: List[str] = None) -> pd.DataFrame:
...
with col_status3:
    if st.session_state.last_scan_duration:
        st.info(f"⏱️ Duration: {st.session_state.last_scan_duration:.1f}s")
...
with st.sidebar:
    ...
    if not st.session_state.oanda_available:
        ...
        st.caption(f"⚠️ Limit: ~{max_days} days for {cfg.LTF}")
    ...
    # Signal management
    st.subheader("🗂️ Signal Management")
    ...
    with col_arch2:
        if st.button("Delete Archived"):
            ...
            st.success(f"Deleted {count} signals")
...
# ────────────────────────────────────────────────
with tab2:
    st.header("Historical Scan & Backtest Results")
